...
The ingress runtime is part of the `warden-ingress` subsystem, while
`warden-server` remains only the composition root that wires and starts it.

Current implementation status as of March 26, 2026:

- `warden-ingress` has already been split into `warden-ingress`,
  `warden-ingress-types`, `warden-ingress-http`, `warden-ingress-proxy`,
  `warden-ingress-resolver`, and `warden-ingress-stream`
- the runtime already consumes a compiled `IngressRouteSnapshot` rather than
  reading raw `RouteRecord` values directly on the request path
...
`warden-server` should not own ingress routing logic. It should only construct,
...
## Crate Split Direction

The ingress package split can go to the target shape directly because the
current crate is still small and its boundaries are already visible.

Recommended crate layout:

- `warden-ingress`
  Facade crate and service entrypoint for the ingress subsystem
- `warden-ingress-types`
  Shared route snapshot, backend reference, listener config, and runtime-facing
  ingress types
- `warden-ingress-http`
  HTTP/HTTPS route matching and request dispatch
- `warden-ingress-proxy`
  Reverse proxy behavior, header forwarding, websocket, streaming, and timeout
  handling
- `warden-ingress-resolver`
  Backend resolution from registry and endpoint health state
- `warden-ingress-stream`
  Later crate for TCP/UDP ingress if and when stream ingress becomes a real
  productized path
...
## Relationship to Current `warden-ingress` Crate

The current crate already contains:

- HTTP proxy path
- TCP/UDP stream proxy helpers
- ingress service bootstrap

The design direction should evolve that crate into a clearer structure:

- keep HTTP ingress runtime and reverse proxy as the main v1 focus
- move route compilation and backend selection into explicit layers
- treat TCP/UDP stream ingress as later expansion, not the primary v1 path

In other words, the crate should converge toward:

- control-plane route snapshot consumption
- per-node ingress runtime
- reverse proxy implementation

rather than remaining a collection of loosely connected proxy helpers.
...
3. Split `warden-ingress` into `warden-ingress`, `warden-ingress-types`,
   `warden-ingress-http`, `warden-ingress-proxy`, and `warden-ingress-resolver`
