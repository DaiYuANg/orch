...
func (p *Provider) Status(ctx context.Context, meta deployv1.Metadata, workloadName string) (runtimeinfo.Status, error) {
	...
	out := stoppedDockerStatus(workloadName)
	if len(containers) == 0 {
		return out, nil
	}
...
func stoppedDockerStatus(workloadName string) runtimeinfo.Status {
	return runtimeinfo.Status{Name: strings.TrimSpace(workloadName), Runtime: deployv1.RuntimeDocker, Status: "stopped"}
}
...
func (p *Provider) readDockerLogs(ctx context.Context, cli *client.Client, containerID string, opts runtimeinfo.LogOptions) (string, error) {
	...
	if err != nil {
		return "", oopsx.B("runtime", "docker").Wrapf(err, "docker logs")
	}
	defer func() {
	...
	return decodeDockerLogs(reader)
}

func decodeDockerLogs(reader io.Reader) (string, error) {
	var stdout, stderr bytes.Buffer
	if _, err := stdcopy.StdCopy(&stdout, &stderr, reader); err != nil {
		return "", oopsx.B("runtime", "docker").Wrapf(err, "docker logs decode")
	}
...
func (p *Provider) newDockerClient() (*client.Client, error) {
	...
	if err != nil {
		return nil, oopsx.B("runtime", "docker").Wrapf(err, "docker client")
	}
	return cli, nil
}
