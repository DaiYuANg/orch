...
| `containerd` | Linux with containerd CRI plugin and working CNI | Yes | Yes | Yes | Injects orch DNS through CRI sandbox DNS config and records workload A records from sandbox IP | Uses CRI pod sandboxes only. `CONTAINERD_ADDRESS` can override the default socket. |
