...
func NewService(logger *slog.Logger, metricService *metrics.Service, runtimeManager *runtime.Manager, registryService *registry.Service, cfg config.Config, catalog *nodecapacity.Catalog) *Service {
	return &Service{
		logger:   logger,
		cfg:      cfg,
...
func localNodeID(cfg config.Config) string {
	id := strings.TrimSpace(cfg.Raft.Node.ID)
	if id == "" {
		return "local"
	}
	return id
...
func (s *Service) DeployApp(ctx context.Context, app *deployv1.App) error {
	...
	if err := s.catalog.RefreshLocal(ctx, s.cfg); err != nil {
		s.logger.Warn("refresh local node capacity before placement", "error", err)
	}
	self := localNodeID(s.cfg)

	for i := range app.Workloads {
		w := &app.Workloads[i]
		chosen, err := placement.Choose(ctx, *w, s.catalog, self)
		if err != nil {
